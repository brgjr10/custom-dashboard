...
- **Modular widgets** — System stats, Docker health, network info, speed test, weather, GitHub activity, quick links, Pi-hole, custom HTML
...
| `/api/pihole` | Pi-hole stats (queries, blocked, block rate, clients) |
...
The Pi-hole widget connects to your Pi-hole instance using its v6 API. Configure it via the widget settings UI or environment variables:

| Variable | Description |
|----------|-------------|
| `PIHOLE_BASE_URL` | Base URL of your Pi-hole instance (e.g. `http://192.168.4.90`) |
| `PIHOLE_PASSWORD` | Admin password for Pi-hole v6 API auth |
| `PIHOLE_TOKEN` | Alternative to `PIHOLE_PASSWORD` for API auth |
